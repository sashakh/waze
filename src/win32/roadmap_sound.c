...
DWORD WINAPI SoundThread (LPVOID lpParam) {
   ...
   roadmap_log (ROADMAP_WARNING, "Start SoundThread()");
   ...
   roadmap_log (ROADMAP_WARNING, "Exit SoundThread()");
}
...
DWORD WINAPI SoundRecThread (LPVOID lpParam) {
   ...
   roadmap_log (ROADMAP_WARNING, "Start SoundRecThread()");
   while (1) {
   ...
   roadmap_log (ROADMAP_WARNING, "Exit SoundRecThread()");
}
...
int roadmap_sound_play_file (const char *file_name)
{
	...
	if (roadmap_path_is_full_path (file_name)) {
		file_name_unicode = ConvertToUNICODE(file_name);
	} else {
		snprintf (full_name, sizeof(full_name), "%s\\sound\\%s",
				roadmap_path_user (), file_name);

		file_name_unicode = ConvertToUNICODE(full_name);
	}
...
/**
 * @brief Create a RoadMapSound structure from a file
 * @param path name of directory containing the file
 * @param file the file name
 * @param mem pointer to an int that'll contain the size of this sound
...
RoadMapSound roadmap_sound_load (const char *path, const char *file, int *mem)
{
	...
	seq = roadmap_file_map (NULL, full_name, "r", &sound);
...
/**
 * @brief unmap/release the file associated with this RoadMapSound structure
 * @param sound
...
{
	RoadMapSoundList list = (RoadMapSoundList)
		calloc (1, sizeof(struct roadmap_sound_list_t));
