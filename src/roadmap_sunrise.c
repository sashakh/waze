...
static time_t roadmap_sunrise_getriseorset
   ...
   double longitude,l,gx,gha,correction,ec,lambda,delta,e,obl,cosc;
	double altitude,ephem2000day,utold,utnew;
   double days,t;
	int count = 0;
   ...
   curtime_gmt = *(gmtime(&gmt));
	...
	ephem2000day = 367.0 * (curtime.tm_year+1900)
                      - 7.0 * ((curtime.tm_year+1900)
                                  + ((curtime.tm_mon+1) + 9.0) / 12.0) / 4.0
                      + 275.0 * (curtime.tm_mon+1) / 9.0
                      + curtime.tm_mday - 730531.5;
	utold = PI;
	utnew = 0;
	
   sinalt = sin(altitude * RADIANS);
	sinphi = sin(roadmap_lat * RADIANS);
	cosphi = cos(roadmap_lat * RADIANS);
	longitude = (roadmap_long * RADIANS);
