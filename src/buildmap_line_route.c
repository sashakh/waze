...
int  buildmap_line_route_add
   ...
   if (from_flags != ROUTE_CAR_ALLOWED) from_avg_speed = 0;
   if (to_flags != ROUTE_CAR_ALLOWED) to_avg_speed = 0;
         ...
         for (j=0; j<2; j++) {
            ...
            for (res=0; res<count; res++) {
               ...
               if (line_from == point_id) {
                  avg_speed = line_route->record.from_avg_speed;
               } else if (line_to == point_id) {
                  avg_speed = line_route->record.to_avg_speed;
               } else {
               ...
               if (avg_speed) {
                  if ((res_bit < 8) &&
                     ...
                     printf("Found restriction: bit:%d, node:%d, from:%d, to:%d\n", res_bit, point_id, one_route->line, line);
                  }
                  res_bit++;
                  if (res_bit > 8) {
