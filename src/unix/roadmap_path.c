...
char *roadmap_path_expand (const char *item, int length) {
   ...
   res = malloc (length + expand_length + 1);
   roadmap_check_allocated(res);

   strcpy (res, expand);
   strncat (res, item, length);

   res[length+expand_length] = 0;
