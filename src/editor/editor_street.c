...
static int editor_street_get_distance_with_shape
   ...
   current.cfcc = cfcc;
   current.fips = fips;
   current.plugin_id = EditorPluginID;


   smallest_distance = 0x7fffffff;
   current.from = from;

   editor_trkseg_get (trkseg, &i, NULL, &first_shape, &last_shape, NULL);
...
int editor_street_get_distance (const RoadMapPosition *position,
   ...
   return editor_street_get_distance_with_shape (position, line, fips, result);
...
int editor_street_get_closest (const RoadMapPosition *position,
   ...
   square_count =
      editor_square_find_by_position (position, square, 5, distance);
...
int editor_street_get_connected_lines (RoadMapPosition *crossing,
                                       int *plugin_lines,
                                       int size) {
