...
void editor_screen_short_click (RoadMapGuiPoint *point) {
   ...
   if (roadmap_navigate_retrieve_line (&position, 20, &closest) == -1) {
   ...
   for (i=0; i<select_count; i++) {
      if ((SelectedLines[i].line == closest.line) &&
            (SelectedLines[i].plugin_id == closest.plugin_id) &&
            (SelectedLines[i].fips == closest.fips)) break;
   }
   ...
   SelectedLines[select_count].line = closest.line;
   SelectedLines[select_count].cfcc = closest.cfcc;
   SelectedLines[select_count].plugin_id = closest.plugin_id;
   SelectedLines[select_count].fips = closest.fips;
   select_count++;
...
int editor_override_line (int line, int first_shape, int last_shape,
                          int cfcc, int pen_type, RoadMapPen *override_pen) {
   ...
   ActiveFips = roadmap_locator_active ();
   ...
   for (i=0; i<select_count; i++) {
      if ((line == SelectedLines[i].line) &&
          (SelectedLines[i].plugin_id != EditorPluginID) &&
          (ActiveFips == SelectedLines[i].fips)) {
         is_selected = 1;
         break;
      }
   }
   ...
   if (is_selected) {
      ...
      road_state = editor_screen_get_road_state (line, 0, ActiveFips);
   }

   if (road_state != NO_ROAD_STATE) {

      if (pen_type > 1) return 1;
   ...
   for (i=0; i<count; i++) {
      ...
      if (editor_is_enabled () ) {
         ...
         for (j=0; j<select_count; j++) {
            ...
            if ((line == SelectedLines[j].line) &&
                (SelectedLines[j].plugin_id == EditorPluginID)  &&
                (fips == SelectedLines[j].fips)) {
      ...
      editor_trkseg_get (trkseg, &j, NULL, &first_shape, &last_shape, NULL);
