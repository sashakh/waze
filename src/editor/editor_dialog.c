...
static const char *editor_segments_find_city
   ...
   for (i = 0; i < count; ++i) {
      ...
      if (neighbours[i].plugin_id == EditorPluginID) {
         EditorStreetProperties properties;

         editor_street_get_properties
            (neighbours[i].line, &properties);
...
static void editor_segments_apply (const char *name, void *context) {
   ...
   for (i=0; i<selected_lines->count; i++) {
      ...
      if (line->plugin_id == EditorPluginID) {
         ...
         editor_line_modify_properties (line->line, cfcc, 0);
      } else { /* != Editor line */

         int new_line = editor_line_copy (line->line, line->cfcc, line->fips);
         if (new_line == -1) {
            editor_log
               (ROADMAP_ERROR,
      ...
      editor_line_get_street (line->line, &street_id, &street_range_id);
      ...
      editor_line_set_street (line->line, &street_id, &street_range_id);

      route_id = editor_line_get_route (line->line);

      if (route_id == -1) {
         route_id = editor_route_segment_add (0, 0);
         editor_line_set_route (line->line, route_id);
      } else {
   ...
   if (selected_lines->count > 1) {
      ...
      for (i=0; i < count; i++) {
         lines[i] = selected_lines->lines[i].line;
      }
...
void editor_segments_fill_dialog (SelectedLine *lines, int lines_count) {
   ...
   for (i=0; i<lines_count; i++) {
      ...
      if (lines[i].plugin_id == EditorPluginID) {
         if (editor_db_activate (lines[i].fips) != -1) {

            EditorStreetProperties properties;
            editor_street_get_properties (lines[i].line, &properties);
            ...
            route_id = editor_line_get_route (lines[i].line);
         ...
         roadmap_locator_activate (lines[i].fips);
         roadmap_street_get_properties (lines[i].line, &properties);
         ...
         route_id = editor_override_line_get_route (lines[0].line);
   ...
   if (same_l_city && same_r_city && !strlen(l_city) && !strlen(r_city)) {
      ...
      l_city = r_city = editor_segments_find_city
                           (lines[0].line, lines[0].plugin_id, lines[0].fips);
   }
...
void editor_segments_properties (SelectedLine *lines, int lines_count) {
   ...
   for (i=0; i<selected_lines->count; i++) {
      ...
      if (line->plugin_id == EditorPluginID) {
         ...
         speed = editor_line_get_avg_speed (line->line, 1);
