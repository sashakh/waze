...
static void buildmap_postgres_read_roads_lines (int verbose) {
   ...
   for (irec=0; irec<record_count; irec++) {
      ...
      prefix = PQgetvalue(db_result, irec, column++);

      if (strlen(prefix)) {
         sprintf(street_name, "%s ", prefix);
      }

      strcat(street_name, PQgetvalue(db_result, irec, column++));

      fename = str2dict (DictionaryStreet, street_name);
